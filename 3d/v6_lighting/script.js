...
function update(cam_pos, yaw, pitch){
    let v = [Math.sin(yaw)*Math.cos(pitch),Math.sin(pitch),Math.cos(yaw)*Math.cos(pitch)]
    let look_at = misc.add_vec(v, cam_pos);
    ...
    //gl.uniform3fv(u_light_direction_loc, v);
...
canvas.addEventListener('click',e=>{if (in_play){cam_pos=(e.button==0?misc.add_vec:misc.sub_vec)(cam_pos,[Math.sin(y),Math.sin(p),Math.cos(y)]);update(cam_pos,y,p);}else{canvas.requestPointerLock();in_play=true;}});
canvas.addEventListener('mousemove',e=>{if (!in_play) return; y += e.movementX * 0.002; p += -e.movementY * 0.002; update(cam_pos,y,p);});
